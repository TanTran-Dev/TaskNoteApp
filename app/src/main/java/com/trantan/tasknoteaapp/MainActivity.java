...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onContextItemSelected(@NonNull MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.edit:
                //show alert dialog edit
                Cursor cursor = (Cursor) customAdapter.getItem(selectedPosition);
                TaskNote selectedTaskNote = TaskNote.getNoteFromCursor(cursor);
                NoteAlertDialog noteAlertDialog = new NoteAlertDialog(MainActivity.this, selectedTaskNote, NoteAlertDialog.UPDATE);
                noteAlertDialog.show();
                break;
            case R.id.delete:
                //Show confirmation Dialog
                new AlertDialog.Builder(this).setTitle("Delete this Note")
                        ...
                        .setPositiveButton(android.R.string.yes, (DialogInterface dialog, int whichButton) -> {
                            ...
                            if (selectedCursor != null) {
                                ...
                                if (taskNote != null) {
                                    //Delete taskNote
                                    NoteModify.getInstance(MainActivity.this).deleteNote(taskNote.getNoteId());
                                    //You must refresh listview after udate
                                    refreshListView();
                                }
                            }
